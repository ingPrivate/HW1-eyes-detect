...
OUTPUT_DIR = os.path.join(BASE_DIR, 'output-png', 'output_v7')
...
def detect_pupil_in_roi(roi_gray, ew, eh):
    ...
    # Ellipse Fitting (Fine-tuned threshold)
    ...
    # Hough Fallback
    circles = cv2.HoughCircles(roi_blurred, cv2.HOUGH_GRADIENT, 1, 30, param1=50, param2=12, minRadius=int(ew*0.05), maxRadius=int(ew*0.3))
...
for img_path in image_paths:
    ...
    # 1. Refined Face Detection (Strict)
    # minNeighbors=6 and minSize help filter out skin texture and noise
    faces = face_cascade.detectMultiScale(gray_en, 1.2, 6, minSize=(150, 150))
    
    pupils = []
    if len(faces) > 0:
        # Optimization: Take only the largest face found (Primary Subject)
        faces = sorted(faces, key=lambda f: f[2]*f[3], reverse=True)
        (fx, fy, fw, fh) = faces[0]
        cv2.rectangle(display_img, (fx, fy), (fx+fw, fy+fh), (255, 255, 0), 2)
        
        # Eye Detection within TOP HALF of the face only
        eye_search_h = int(fh * 0.6)
        face_top_gray = gray_en[fy:fy+eye_search_h, fx:fx+fw]
        eyes = eye_cascade.detectMultiScale(face_top_gray, 1.1, 10, minSize=(fw//10, fw//10))
        
        face_pupils = []
        for (ex, ey, ew, eh) in eyes:
            cv2.rectangle(display_img, (fx+ex, fy+ey), (fx+ex+ew, fy+ey+eh), (0, 255, 255), 1)
...
df.to_csv(os.path.join(CASCADE_DIR, 'final_v7_analysis.csv'), index=False)
print(df.to_string())
